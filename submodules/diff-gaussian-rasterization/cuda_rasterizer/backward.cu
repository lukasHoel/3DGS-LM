	...
	float* dL_dcov)
{
    ...
    // const float lim_x_pos = (width - cx) / h_x + 0.3f * tan_fovx;
    // const float lim_x_neg = cx / h_x + 0.3f * tan_fovx;
    // const float lim_y_pos = (height - cy) / h_y + 0.3f * tan_fovy;
	...
	unsigned balance_threshold)
{
	...
	const uint32_t horizontal_blocks = (W + GSGN_BLOCK_X - 1) / GSGN_BLOCK_X;
	const uint2 pix_min = { block.group_index().x * GSGN_BLOCK_X, block.group_index().y * GSGN_BLOCK_Y };
	const uint2 pix_max = { min(pix_min.x + GSGN_BLOCK_X, W), min(pix_min.y + GSGN_BLOCK_Y , H) };
	...
	__shared__ float collected_colors[C * GSGN_BLOCK_SIZE];
	...
	for (int i = 0; i < rounds; i++, toDo -= GSGN_BLOCK_SIZE)
	{
		...
		block.sync();
		const int progress = i * GSGN_BLOCK_SIZE + block.thread_rank();
		if (range.x + progress < range.y)
		{
			...
			for (int i = 0; i < C; i++)
				collected_colors[i * GSGN_BLOCK_SIZE + block.thread_rank()] = colors[coll_id * C + i];
		}
		block.sync();
		...
		for (int j = 0; !done && j < min(GSGN_BLOCK_SIZE, toDo); j++)
		{
			...
			for (int ch = 0; ch < C; ch++) {
				const float c = collected_colors[ch * GSGN_BLOCK_SIZE + j];
	...
	unsigned balance_threshold)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= GSGN_BLOCK_SIZE)
	{
		...
		// Iterate over Gaussians
		for (int j = 0; j < min(GSGN_BLOCK_SIZE, toDo); j++)
		{
			__syncwarp();
	...
	float* dL_dcolors)
{
	...
	switch(bw_impl) {
		...
		renderCUDABW_original<GSGN_NUM_CHANNELS> <<<grid, block >>>(
			ranges,
			point_list,
			W, H,
			bg_color,
		...
		case 1:
		if (ct == 1) {
			std::cout << "BW IMPLEMENTATION: BUTTERFLY (SW-B)\n";
		}
		renderCUDABW_butterfly<GSGN_NUM_CHANNELS> <<<grid, block >>>(
			ranges,
			point_list,
			W, H,
			bg_color,
		...
		case 2:
		if (ct == 1) {
			std::cout << "BW IMPLEMENTATION: SERIALIZED (SW-S)\n";
		}
		renderCUDABW_serialized<GSGN_NUM_CHANNELS> <<<grid, block >>>(
			ranges,
			point_list,
			W, H,
			bg_color,
